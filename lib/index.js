...
var _underscore, clone, is_object, is_object_literal, merge, mutate,
...
module.exports.underscore = function(source, underscore = true) {
  var name, src, target, u;
  target = {};
  if (is_object_literal(source)) {
    u = typeof underscore === 'number' && underscore > 0 ? underscore - 1 : underscore;
    for (name in source) {
      src = source[name];
      if (underscore) {
        name = _underscore(name);
      }
